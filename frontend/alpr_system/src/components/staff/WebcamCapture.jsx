...
const CAMERA_ID   = 'webcam-laptop'
const INTERVAL_MS = 1000

export default function WebcamCapture({ embedded = false }) {
  ...
  const inner = (
    <div className="space-y-4">
      ...
      <div className="relative bg-surface border border-border rounded overflow-hidden"
        ...
        {!active && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-muted text-sm font-mono">Camera inactive</p>
          </div>
        )}
        {active && (
          <div className="absolute top-2 left-2 flex items-center gap-1.5 bg-bg/80 rounded px-2 py-1">
            <span className="w-1.5 h-1.5 rounded-full bg-red animate-pulse" />
            <span className="text-xs font-mono text-primary">{frameCount} frames</span>
          </div>
        )}
        {active && (
      ...
      <p className="text-xs text-muted font-mono">
        Session-based · Start when vehicle approaches · Stop after detection
      </p>
    </div>
  )
